...
The performance of the Mistral 7B Instruct model was evaluated both with and without RAG augmentation. Responses were compared to an ideal response sourced from the RAG library via cosine similarity. Results are provided in the plot below. The RAG system outperformed the LLM only approach for all questions, but to varying degrees. The LLM struggled to provide concise answers for complex questions without RAG augmentation. The code for this comparison is provided in the "notebooks" folder. The questions used are provided in the "test_questions.txt" text file. 
